%% AeroStabilize - Circle Trajectory Tracking
...
% Start near the beginning of the circle
[x_start, y_start, z_start, ~, ~, ~] = circle_trajectory(0);
...
dynamics = @(t,x) circle_closed_loop( ...
    t, x, ...
    psi_des, ...
    Kp_x, Kd_x, ...
    Kp_y, Kd_y, ...
...
title('AeroStabilize - Circle Trajectory Tracking');
...
plot(t, position_error, 'LineWidth', 1.5);
grid on;

xlabel('Time [s]');
ylabel('Position Error [m]');
title('Circle Trajectory Tracking Error');

function xdot = circle_closed_loop( ...
    t, x, ...
    psi_des, ...
    Kp_x, Kd_x, ...
    Kp_y, Kd_y, ...
...
tau_phi = ...
    Kp_phi*e_phi - Kd_phi*p;

tau_theta = ...
    Kp_theta*e_theta - Kd_theta*q;

tau_psi = ...
    Kp_psi*e_psi - Kd_psi*r;
